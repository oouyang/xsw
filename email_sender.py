...
class EmailSender:
    ...
    def send_email(
        ...
        try:
                ...
                for attachment_path in attachments:
                    try:
                        ...
                        if not file_path.exists():
                            logger.warning(
                                f"[EmailSender] Attachment not found: {file_path}"
                            )
                            continue
                        ...
                        with open(file_path, "rb") as f:
                            ...
                            msg.attach(part)
                            logger.debug(
                                f"[EmailSender] Attached file: {file_path.name}"
                            )
                    except Exception as e:
                        logger.error(
                            f"[EmailSender] Failed to attach file {attachment_path}: {e}"
                        )
            ...
            logger.info(
                f"[EmailSender] Email sent successfully to {', '.join(to_email)}"
            )
            return {
                "status": "success",
