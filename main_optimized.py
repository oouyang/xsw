...
def _bg_fetch_chapters(book_id: str):
    ...
    try:
        ...
        if cached_chapters and len(cached_chapters) > 0:
            print(
                f"[BG] Book {book_id} already has {len(cached_chapters)} chapters cached"
            )
            return
        ...
        items = fetch_chapters_from_liebiao(
            html_content, page_url=url, canonical_base=canonical_base()
        )
...
async def lifespan(app: FastAPI):
    ...
    slow_rate = float(
        os.getenv("MIDNIGHT_SYNC_RATE_LIMIT", "5.0")
    )  # Default: 5s between books
    ...
    periodic_priority = int(
        os.getenv("PERIODIC_SYNC_PRIORITY", "3")
    )  # Default: priority 3
    ...
    print(
        f"[App] Database, cache, {num_workers} background workers, midnight scheduler, and periodic sync (every {periodic_interval}h) initialized"
    )
...
def list_books_in_category(
    ...
    try:
        ...
        if bg_sync and bg_jobs.job_manager:
            ...
            print(
                f"[API] Category {cat_id} page {page}: Queued {queued}/{len(book_ids)} books for background sync"
            )
    ...
    try:
        ...
        if not all_chapters:
            ...
            if www:
                ...
                html_content = fetch_html(home_url)
                items = fetch_chapters_from_liebiao(
                    html_content, home_url, canonical_base()
                )
            else:
            ...
            if all_chapters:
                ...
                print(
                    f"[API] Syncing book {book_id} with last chapter: {last_chapter.number} - {last_chapter.title}"
                )
                ...
                if book_info:
                    ...
                    print(
                        f"[API] Updated book {book_id} last_chapter_number to {last_chapter.number}"
                    )
                else:
                    ...
                    try:
                        ...
                        if info:
                            ...
                            cache_mgr.cache_manager.store_book_info(book_id, info)
                            print(
                                f"[API] Created book info for {book_id} with last_chapter_number: {last_chapter.number}"
                            )
                    except Exception as e:
                        print(
                            f"[API] Warning: Failed to fetch book info during sync: {e}"
                        )
...
def get_chapter_content(
    ...
    try:
        ...
        if not chapter_url:
            ...
            print(
                f"[API] Chapter {book_id}:{chapter_num} - URL not in DB, fetching chapter list"
            )
            ...
            chapter_title = (
                get_chapter_title_from_db(book_id, chapter_num)
                or f"Chapter {chapter_num}"
            )
            print(
                f"[API] Chapter {book_id}:{chapter_num} - found URL in DB: {chapter_url}"
            )
        ...
        cache_mgr.cache_manager.store_chapter_content(
            book_id, chapter_num, content_data
        )
...
def trigger_book_sync(
    book_id: str, priority: int = Query(10, description="Job priority (higher = first)")
):
    """Manually trigger background sync for a specific book."""
    if not bg_jobs.job_manager:
        raise HTTPException(
            status_code=503, detail="Background job manager not available"
        )
...
def admin_init_sync(
    categories_limit: int = Query(
        7, ge=1, le=20, description="Number of categories to scan"
    ),
    pages_per_category: int = Query(
        10, ge=1, le=50, description="Pages to scan per category"
    ),
):
    """
    ...
    try:
        ...
        session.commit()
        print(
            f"[INIT_SYNC] Deleted: {deleted_books} books, {deleted_chapters} chapters, {deleted_categories} categories, {deleted_queue} queue items"
        )
...
def clear_midnight_sync_completed():
    ...
    if not midnight.midnight_scheduler:
        raise HTTPException(
            status_code=503, detail="Midnight sync scheduler not available"
        )
    ...
    return {
        ...
        "message": f"Cleared {cleared} completed/failed entries",
    }
...
def trigger_periodic_sync_now():
    ...
    if not periodic.periodic_scheduler:
        raise HTTPException(
            status_code=503, detail="Periodic sync scheduler not available"
        )
