...
def reset_password(db_path: str, email: str, new_password: str, verbose: bool = True):
    ...
    with engine.connect() as conn:
        ...
        result = conn.execute(
            text(
                "SELECT email, auth_method, is_active FROM admin_users WHERE email = :email"
            ),
            {"email": email},
        )
        ...
        if not user:
            if verbose:
                ...
                result = conn.execute(
                    text("SELECT email, auth_method, is_active FROM admin_users")
                )
...
def list_users(db_path: str):
    ...
    with engine.connect() as conn:
        result = conn.execute(
            text(
                "SELECT email, auth_method, is_active, created_at, last_login_at FROM admin_users"
            )
        )
...
Examples:
  ...
  %(prog)s --list
    ...
    parser.add_argument(
        "--password", default="admin", help="New password (default: admin)"
    )
    parser.add_argument(
        "--list", action="store_true", help="List all admin users and exit"
    )
    parser.add_argument(
